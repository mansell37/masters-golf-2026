...
Team entries, groups, and bonus questions are currently stored in browser `localStorage`, so they are **per browser/device** and not shared globally.

If you want every user to see the same live entries/groups across devices, the next step is to move that data to server-side storage (SQLite/Postgres) behind API endpoints.
